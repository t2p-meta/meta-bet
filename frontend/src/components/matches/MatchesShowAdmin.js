...
class MatchesShowAdmin extends Component {
          ...
          APICall(contractMatch.matchResultLink)
            .then((result) => {
              ...
              if (result) {
                ...
                if (
                  parseInt(match.goalsHomeTeam) > parseInt(match.goalsAwayTeam)
                ) {
                  match.winnerBetOn = 0;
                } else if (
                  parseInt(match.goalsHomeTeam) < parseInt(match.goalsAwayTeam)
                ...
                console.log(
                  " goalsHomeTeam : goalsHomeTeam , winnerBetOn",
                  match.goalsHomeTeam,
                  match.goalsAwayTeam,
                  match.winnerBetOn
                );
                ...
                console.log(" oddsA, oddsB, oddsDraw ", oddsA, oddsB, oddsDraw);
                match.state = parseInt(contractMatch.state); // 0:未开始；1：开始；2：结束
              ...
              <Grid style={{ height: "65%" }} container spacing={2}>
                ...
                <Grid item xs={6}>
                  <Button
                    ...
                    style={{
                      backgroundColor:
                        this.state.match.state % 2 == 0 ? "#595959" : "red",
                      color:
                        this.state.match.state % 2 != 0 ? "#ffffff" : "#878787",
                      fontWeight: "bold",
                      marginBottom: "20px",
                    }}
                    disabled={this.state.match.state % 2 == 0}
                    variant="contained"
                    fullWidth
                  >
                ...
                <Grid item xs={6}>
                  <Button
                    style={{
                      backgroundColor:
                        this.state.match.state != 0 ? "#595959" : "#357a38",
                      color:
                        this.state.match.state == 0 ? "#ffffff" : "#878787",
                      fontWeight: "bold",
                    }}
                    disabled={this.state.match.state != 0}
                    variant="contained"
                    fullWidth
  ...
  closeMatch = () => {
    try {
      ...
      if (this.state.match.winnerBetOn == 0) {
        //teamA
        winnerBetOn = 2;
      } else if (this.state.match.winnerBetOn == 1) {
        //teamB
        winnerBetOn = 3;
      } else if (this.state.match.winnerBetOn == 2) {
        //draw
        winnerBetOn = 1;
      } else {
      ...
      this.state.contract.methods
        .closeMatch(
          ...
          winnerBetOn,
          0,
          this.state.match.goalsHomeTeam,
          this.state.match.goalsAwayTeam
        )
        .send({
          from: this.state.account,
        })
  ...
  getImageSection = (teamIndex) => {
    ...
    const borderStyle = {
      ...
      border:
        this.state.match.winnerBetOn === teamIndex
          ? `5px #FFD700 solid`
          : "none",
    };
